                    ...
                    unsigned char *output )
{
    ...
    else if(ctx->mode == SM4_DECRYPT)/* SM4_DECRYPT */
    {
        while( length > 0 )
        {
 ...
 */
static void sm4_crypt_ctr( sm4_context *ctx,
								int length,
								unsigned char iv[16],
								unsigned char *input,
								unsigned char *output)
...
void sm4_ecb_cryption(u8 *key, int enc, int dsize, u8 *in, u8 *out)
{
    sm4_context ctx;
    ctx.mode = enc;
    ...
    /*cbc*/
    sm4_crypt_ecb(&ctx, dsize, in, out);
}
void sm4_cbc_cryption(u8 *key, int enc, u8 iv[16], int dsize, u8 *in, u8 *out)
{
    sm4_context ctx;
    ctx.mode = enc;

    /*cbc mode encrypt, set key enc/dec*/
    if(ctx.mode == SM4_ENCRYPT)
    {
        sm4_setkey_enc(&ctx, key);
    }
    ...
    /*cbc*/
    sm4_crypt_cbc(&ctx, dsize, iv, in, out);
}

void sm4_ctr_cryption(u8 *key, int enc, u8 iv[16], int dsize, u8 *in, u8 *out)
{
	sm4_context ctx;
	ctx.mode = enc;
	/*set key*/
	sm4_setkey_enc(&ctx, key);
	/*cbc*/
	sm4_crypt_ctr(&ctx, dsize, iv, in, out);
}
