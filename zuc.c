...
/* initialize */
static void Initialization(u8* k, u8* iv)
{
	u32 u, nCount;
...
static void GenerateKeystream(u32* pKeystream, int KeystreamLen)
{
	int i;
...
static void ZUC(u8* k, u8* iv, u32* ks, int len)
{
...
#ifdef ZUC_IV_CONF
static void EEA3(u8* CK, u32 COUNT,u32 BEARER,u32 DIRECTION,u32 LENGTH,u32* M,u32* C)
{
	u32 *z, L, i;
	u8 	IV[16];
...
#else
static void EEA3(u8* CK,u8 *IV,u32 LENGTH,u32* M,u32* C)
{
	u32 *z, L, i;
...
void zuc_encryption(u8 *key, u8 iv[16], u32 nbytes, u8 *in, u8 *out)
{
	u32 count;
	u32 bearer;
	u32 direction;

	count = (iv[3] << 24) | (iv[2] << 16) | (iv[1] << 8) | iv[0];
	bearer = iv[4] & 0x1f;
	direction = iv[5] & 1;

	EEA3(key, count, bearer, direction, 8*nbytes, (u32 *)in, (u32 *)out);
}
